...
describe('analyzeWordPressExport', () => {
  ...
  it('carries the site name and address out of the file', async () => {
    const report = await analyze([], {
      title: 'Crem Coffee',
      baseBlogUrl: 'https://crem.test',
    });

    expect(report.siteTitle).toBe('Crem Coffee');
    expect(report.sourceUrl).toBe('https://crem.test');
  });
