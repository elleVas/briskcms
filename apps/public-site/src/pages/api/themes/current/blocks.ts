...
// Docs/adr/0041, same pattern/reasoning as icons.ts and
// block-style-defaults.ts: resolved here because only apps/public-site
// has a real ~theme alias — called by editor-app once per session to
...
// resolve-theme-block-style-defaults.ts). The actual, reliable
// collision gate is each theme's own blocks.spec.ts (see
...
export const GET: APIRoute = () =>
  new Response(JSON.stringify(listThemePageBlocks([])), {
    status: 200,
    headers: {
