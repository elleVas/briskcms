...
export function PageEditorView({ pageId }: PageEditorViewProps) {
  ...
  return (
    ...
    // IconListProvider backs any block with an icon-picker field
    // (currently Feature and NavLink) — this view was missing it entirely
    // until a real crash surfaced it: editing Feature's icon threw
    // "useIconList() called outside an IconListContext.Provider" because
    // only site-layout-section-editor-view.tsx (built first, when only
    // NavLink needed it) ever mounted this provider. Any current or
    // future icon-picker block needs it wherever blocks render, not just
    // in the one editor view that happened to need it first.
